...
from utils_image import calculate_psnr, calculate_mae, calculate_rmse, calculate_ssim
...
def insert_adapter(model, key):
    ...
    for m in model.modules():
        if isinstance(m, nn.Linear):
            ...
            indices = key[i].indices().to(weight.device)
            mask = torch.zeros_like(weight).bool()
            mask[indices[0], indices[1]] = True
        
            weight[mask] = 0
            values = key[i].to_dense().to(weight.device)

            m.weight.data = weight + values

            i += 1
    return retained_weight


def metrics(f_list, imgs, gt_imgs):
    logs = []
    names = ['cover'] + [f'secret{i + 1}' for i in range(len(gt_imgs) - 1)]
    for i, (img, gt_img) in enumerate(zip(imgs, gt_imgs)):
        for f in f_list:
            metric_name = names[i] + '_' + f.__name__.split('_')[-1]
            logs.append(metric_name + ':' + '{:.4f}'.format(f(img, gt_img)))
    return ', '.join(logs)
...
class TrainerINR():
    ...
    def __init__(self, device, function_representation,
                 data_converter, secret_data_converter, lr=2e-4, lr_disc=2e-4, betas=(0.5, 0.999),
                 r1_weight=0, max_num_points=None, is_voxel=False,
                 is_point_cloud=False, is_era5=False, print_freq=1, save_dir='',
                 model_save_freq=0, keys=None, fixed=None, loss_weight=None, merge_secret_image=False):
        ...
        self.secret_data_converter = secret_data_converter
        ...
        self.merge_secret_image = merge_secret_image
    ...
    def _save_data_samples_from_representation(self, filename, data_converter=None):
        """
        """
        ...
        with torch.no_grad():
            ...
            samples = []
            samples.append(self.function_representation.sample_grid(data_converter)) # list
    ...
    def train(self, dataloader, secret_dataloader, epochs):
        """
        """
        if self.save_dir:

            # add for INR: save data from function representation
            self._save_data_samples_from_representation("samples_INR_cover_{}.png".format(0))

            for i_key, key in enumerate(self.keys):
                insert_adapter(self.function_representation, key)
                self._save_data_samples_from_representation("samples_INR_secret{}_{}.png".format(i_key + 1, 0), self.secret_data_converter)

            if self.merge_secret_image:
                tmp = Image.open(os.path.join(self.save_dir, "samples_INR_secret{}_{}.png".format(1, 0)))
                width, height = tmp.size
                pred_img = np.empty((int(np.sqrt(len(self.keys)) * height),
                                      int(np.sqrt(len(self.keys)) * width), 3), dtype=np.uint8)

                for i_key in range(len(self.keys)):
                    img_part = Image.open(os.path.join(self.save_dir, "samples_INR_secret{}_{}.png".format(i_key + 1, 0)))
                    index = i_key
                    i = index // 4
                    j = index - (4 * i)
                    pred_img[i * height : (i + 1) * height,
                              j * width : (j + 1) * width, :] = np.array(img_part)
                Image.fromarray(pred_img).save(os.path.join(self.save_dir, 'samples_INR_secret_merged_{}.png'.format(0)))

            insert_adapter(self.function_representation, self.fixed)
        ...
        for epoch in range(epochs):
            ...
            for i, (cover_batch, secret_batch) in enumerate(zip(dataloader, secret_dataloader)):
                self.train_batch(cover_batch, secret_batch)
            ...
            if epoch % self.print_freq == 0:
                self._save_data_samples_from_representation("samples_INR_cover_{}.png".format(epoch))

                for i_key, key in enumerate(self.keys):
                    insert_adapter(self.function_representation, key)
                    self._save_data_samples_from_representation("samples_INR_secret{}_{}.png".format(i_key + 1, epoch), self.secret_data_converter)

                insert_adapter(self.function_representation, self.fixed)

                if self.merge_secret_image:
                    tmp = Image.open(os.path.join(self.save_dir, "samples_INR_secret{}_{}.png".format(1, epoch)))
                    width, height = tmp.size
                    pred_img = np.empty((int(np.sqrt(len(self.keys)) * height),
                                        int(np.sqrt(len(self.keys)) * width), 3), dtype=np.uint8)

                    for i_key in range(len(self.keys)):
                        img_part = Image.open(os.path.join(self.save_dir, "samples_INR_secret{}_{}.png".format(i_key + 1, epoch)))
                        index = i_key
                        i = index // 4
                        j = index - (4 * i)
                        pred_img[i * height : (i + 1) * height,
                                j * width : (j + 1) * width, :] = np.array(img_part)
                    Image.fromarray(pred_img).save(os.path.join(self.save_dir, 'samples_INR_secret_merged_{}.png'.format(epoch)))


                imgs = [np.array(Image.open(os.path.join(self.save_dir, i))) for i in ["samples_INR_cover_{}.png".format(epoch)] + [
                                                            "samples_INR_secret{}_{}.png".format(i_key + 1, epoch) for i_key in range(len(self.keys))
                                                         ]]

                gt_imgs = [np.array(Image.open(dataloader.dataset.samples[i][0])) for i in range(len(dataloader.dataset.samples))] +  \
                         [np.array(Image.open(secret_dataloader.dataset.samples[i][0])) for i in range(len(secret_dataloader.dataset.samples))]   

                logs = metrics([calculate_psnr, calculate_mae, calculate_rmse, calculate_ssim],
                               imgs, gt_imgs)

                self.epoch_logs.append("INR Epoch:{}, {}".format(epoch + 1, logs))

                with open(self.save_dir + '/epoch_logs.txt', 'w') as f:
                    f.writelines('\n'.join(self.epoch_logs))
                print("INR Epoch:{}, {}".format(epoch + 1, logs))

                    

    def train_batch(self, batch, secret_batch):
        ...
        feature_cover = features[0] # just use one sample
        inr_loss = self._train_function(feature_cover)
        ...
        # Extract coordinates and features from data
        coordinates, features = self.secret_data_converter.batch_to_coordinates_and_features(data)
        ...
        for i_key, key in enumerate(self.keys):
            insert_adapter(self.function_representation, key)
            inr_loss_key.append(self._train_function(features[i_key], self.secret_data_converter))

        inr_loss = [inr_loss] + inr_loss_key

        loss = []
        for i in range(len(self.loss_weight)):
            loss.append(self.loss_weight[i] * inr_loss[i])
    ...
    def _train_function(self, features, data_converter=None):
        """
               Args:
        ...
        generated_features = self.function_representation.sample_features(data_converter)

        if data_converter.normalize_features and self.is_voxel:
            # [-1, 1] -> [0, 1]
