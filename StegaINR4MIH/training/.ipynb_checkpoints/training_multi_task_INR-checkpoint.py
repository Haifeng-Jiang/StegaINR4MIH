...
class TrainerINR():
    ...
    def __init__(self, device, function_representation,
                 ...
                 model_save_freq=0, key1=None, key2=None, fixed=None, loss_weight=None):
    ...
    def train(self, dataloader, epochs):
        ...
        if self.save_dir:
            ...
            insert_adapter(self.function_representation, self.key1)
            self._save_data_samples_from_representation("samples_INR_secret1_{}.png".format(0))

            insert_adapter(self.function_representation, self.key2)
            self._save_data_samples_from_representation("samples_INR_secret2_{}.png".format(0))
        ...
        for epoch in range(epochs):
            ...
            if epoch % self.print_freq == 0:
                ...
                insert_adapter(self.function_representation, self.key1)
                self._save_data_samples_from_representation("samples_INR_secret1_{}.png".format(epoch))

                insert_adapter(self.function_representation, self.key2)
                self._save_data_samples_from_representation("samples_INR_secret2_{}.png".format(epoch))

                insert_adapter(self.function_representation, self.fixed)

                imgs = [np.array(Image.open(os.path.join(self.save_dir, 
                                                         i.format(epoch)))) for i in ["samples_INR_cover_{}.png", 
                                                                                    "samples_INR_secret1_{}.png", 
                                                                                    "samples_INR_secret2_{}.png"]]

                gt_imgs = [np.array(Image.open(dataloader.dataset.samples[i][0])) for i in range(3)]
    ...
    def train_batch(self, batch):
        ...
        feature_secret1= features[1] # just use one sample
        feature_secret2 = features[2] # just use one sample
        ...
        #############################################
        insert_adapter(self.function_representation, self.key1)
        inr_loss1 = self._train_function(feature_secret1)
        ...
        #############################################
        insert_adapter(self.function_representation, self.key2)
        inr_loss2 = self._train_function(feature_secret2)

        loss = self.loss_weight[0] * inr_loss +self.loss_weight[1] * inr_loss1 + self.loss_weight[2] * inr_loss2
        loss.backward()
